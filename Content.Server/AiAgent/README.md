...
3. **`NodeLimit = 512`** (`PathfindingSystem.Common.cs`). A* просто перестаёт разворачивать граф.
   ...
   Отсюда `Route.cs`: длинная дорога режется на ноги по навигационным маякам (ширина в глубину по
   графу «маяки не дальше 22 тайлов»), каждая нога укладывается в лимит. Промежуточные ноги модели
   не показываются — она просила отсек, а не пересадку.
...
`use` — один инструмент вместо трёх. `InteractionSystem.UserInteraction` это полный путь левого
клика: сам разбирает, пусты ли руки и что за цель, и разводит по `InteractHand` / `InteractUsing` /
`InteractionActivate`, сам проверяет дальность, доступ и `ActionBlocker`. Плюс `pickup`, `drop`,
`hit`, `module` (смена набора инструментов в руках через `SelectModule`).
...
Живьём (боевой конфиг, Packed): шасси активно, `look` отдаёт 80 объектов с хендлами,
`step восток 3` — «дошёл», X 32.5 → 35.5; `goto Bridge` строит маршрут по маякам, путь 17
полигонов, флаги `Prying, Climbing, Interact`.
...
* **Робот застревает** на некоторых участках: маршрут построен и он по нему идёт, но у части
  шлюзов топчется. `NudgeStuck` написан и собран, но на боевом сервере не проверен.
