...
| OAuth подписок (Codex, Grok) | `~/.cli-proxy-api/` — владеет CLIProxyAPI, см. правило ниже |
...
Потребителей у нас три: `codex` в терминале, hermes-gateway и теперь игровой сервер. Поэтому
токеном владеет **только CLIProxyAPI**, а все трое ходят через него — игровой сервер профилями
`codex` и `grok` на `127.0.0.1:8317`. Логиниться напрямую из `codex` CLI после этого нельзя: это
отзовёт токен у моста и остановит агента посреди раунда, причём проявится это не ошибкой сборки, а
профилем в состоянии «МЁРТВ» в `aiagent llm`.

Немецкий выход (`xray-de`, SOCKS `127.0.0.1:10808`) для chatgpt и x.ai несёт сам мост — дроп-ином
systemd, как уже сделано для hermes. Игровой сервер прокси не наследует: у него он задаётся по
...
3. **`NodeLimit = 512`** (`PathfindingSystem.Common.cs`). A* просто перестаёт разворачивать граф.
   ...
   Первая попытка резала дорогу по навигационным маякам и работала плохо: маяки расставлены по
   смыслу, а не по проходимости, и цепочка «ближайших» упиралась в запертые отсеки — робот вставал
   на пересадке, хотя до цели оставались проходимые коридоры.

   Поэтому **свой поиск** — `BorgPathfinder.cs`. Ищет по `NavMapComponent`: побитовая карта всей
   станции (пол, стены, шлюзы, чанки 8×8), которую игра уже строит для наручного навигационного
   планшета. Ни одного broadphase-запроса — полный поиск через станцию дешевле одного `look`.
   Найденный путь режется на ноги по шесть тайлов и отдаётся апстримовому рулевому: **глобальный
   маршрут наш, локальное движение чужое и проверенное** — физика, обход мебели и людей, двери.
   Соседи только по четырём сторонам: диагональ дала бы выигрыш в проценты, а стоила бы проверок
   срезанных углов, то есть робота, застрявшего в дверном косяке.

   Замер на стенде: путь между двумя самыми далёкими маяками Box — 219 тайлов при 218 по прямой,
   37 ног. На бою Bar → Bridge — «дошёл».

Плюс `NudgeStuck`: если робот три проверки подряд топчется на месте, он **жмёт на ближайшую
закрытую дверь**. Рулевой прокладывает путь через шлюз, но открыть его должен либо бампом (нужен
контакт под нужным углом), либо долгим отжатием; в игре это выглядело как «маршрут построен, путь
17 полигонов, робот полминуты топчется в метре от стеклянного шлюза». Доступ по ID у борга есть,
так что нажатие её просто открывает.
...
* **Две сессии на одном слоте llama-server** бьют префикс-кэш друг друга. `AgentBody.LlmChain`
  заведён под пер-агентную цепочку профилей, но ещё не читается клиентом; альтернатива —
  `--parallel 2` на llama-server (по 131k контекста каждому вместо общих 262k).
* **Отладчик показывает одного агента** — того, кто занял тело последним. Выбор агента в его UI не
  сделан; кадры на шину при этом уходят с настоящим `body.Id`.
...
`ai_data/` в корне репозитория, в git не попадает: `SOUL.md`, `memory/MEMORY.md`,
`people/`, `skills/`, `sessions/`, `logs/`, `bench/`. Лежит вне исходников намеренно — апстримовый
