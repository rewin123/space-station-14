...
4. **Этап 3 — производительность.** Квадратичный `look` — **сделано 16.08, см. ниже**. Осталось:
   автосейв с главного потока, `Nearest()` Левенштейном.
...
```bash
dotnet test Content.AiBench/Content.AiBench.csproj      # 303 зелёных, 8 пропущено, ~8 минут
