...
```bash
Tools/aibench                # все
Tools/aibench Look           # по подстроке имени
Tools/aibench --no-build     # без пересборки
```

`Content.AiBench` поднимает настоящий сервер в процессе через `PoolManager` из
`Content.IntegrationTests` и дёргает **тот же реестр инструментов**, что и петля агента —
через `StationAiAgentSystem.InvokeToolForTest`. Утверждения делаются о **состоянии мира**, а не
об отчёте инструмента: тул, который вернул `ok:true`, пока дверь осталась закрытой, — это ровно
тот баг, который надо ловить, и ловится он только вычиткой мира после.

Модель подменяется через `AiTestHooks.LlmFactory` на `ScriptedLlmClient`, так что GPU и живой
llama-server не нужны.

## Ещё грабли

**7. `CreateTestMap` кладёт РОВНО ОДИН тайл.**
Всё, что заспавнено рядом, висит над пустотой. Читается это как `unpowered` и `not_visible` и
выглядит в точности как баг агента, пока не пойдёшь проверять. `AiWorld` сначала стелет площадку
плитинга 13×13, и только потом что-то спавнит.

**8. Питание в тестах.**
Апстримовый приём (см. `GravityGridTest`) — выставить `ApcPowerReceiverComponent.NeedsPower = false`.
Строить APC с кабельной сетью ради теста двери незачем.

**9. `RA0002` запрещает и запись в поля компонентов, и вызов методов на них.**
`door.State.ToString()` — это «Execute»-доступ, а у нас только «Read»: копируйте в локальную
переменную. Писать `StationAiWhitelistComponent.Enabled` напрямую тоже нельзя — надо звать
`SharedStationAiSystem.SetWhitelistEnabled`, что и правильнее: этот метод несёт апстримовое
оповещение ИИ.

**10. System.Text.Json по умолчанию экранирует кириллицу в `\uXXXX`.**
Для русскоязычного агента это вшестеро больше байт и токенизация как у пунктуационного мусора
вместо слов. Поймано бенчмарком `Inspect_ReportsCutAiWire`, где совершенно верный ответ приехал
как `"провод перерезан"`.
Лечится `Encoder = JavaScriptEncoder.UnsafeRelaxedJsonEscaping` — он задан один раз в
`LlmJson.Options` и `LlmJson.WriterOptions`, которыми обязаны пользоваться все пути сериализации.

**11. `[SetUpFixture]` действует только внутри своей сборки.**
Свой тестовый проект получает `Pool manager has not been initialized` на первом же тесте, пока не
заведёт собственный аналог `PoolManagerTestEventHandler` — у нас это `BenchSetup`.
