...
public sealed class SkillStore
{
    ...
    public bool TryGet(string name, out Skill skill) => _skills.TryGetValue(Normalise(name), out skill!);
    ...
    public void LoadFromDisk()
    {
        ...
        try
        {
            ...
            // Archived skills live in .archive/ and are deliberately not loaded — they are out of
            // the index but recoverable by hand.
    ...
    public SkillResult Write(string name, string when, string body)
    {
        ...
        // Duplicate stopper. Prompt-level pleading did not work on the mcbot deployment — the model
        // kept creating safe_mine_ore next to mine_ore — so this is mechanical.
        ...
        {
            return new SkillResult(false, $"не удалось записать: {e.Message}");
        }
    ...
    public SkillResult Edit(string name, string oldText, string newText)
    {
        ...
        if (!_skills.TryGetValue(name, out var skill))
            return new SkillResult(false, $"нет скилла '{name}'", Nearest(name));
    ...
    public string RenderIndex()
    {
        ...
        // Sorted so the block is a deterministic function of the library — an unstable order here
        // would change zone 0 on every rebuild for no reason.
