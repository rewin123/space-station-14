...
public sealed partial class AiBorgSystem
{
    ...
    /// <remarks>
    /// <para>
    /// <b>На ходу разность не считается вовсе, и это главное решение в файле.</b> У неподвижного
    /// глаза «появилось/исчезло» — чистый сигнал. У идущего робота смена десяти тайлов означает,
    /// что появилась и исчезла половина станции: строки вытеснили бы из очереди обращение по
    /// рации — то есть ровно ту реплику, ради которой очередь и существует. Поэтому пока робот
    /// идёт, база сравнения просто обновляется молча, а сводка «что вокруг» выдаётся по прибытии.
    /// </para>
    /// <para>
