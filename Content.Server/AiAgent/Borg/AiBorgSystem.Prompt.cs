...
public sealed partial class AiBorgSystem
{
    ...
    private string BuildBorgPrompt(EntityUid borg, AiBorgComponent comp, bool scripted, Vfs.Vfs? vfs = null)
    {
        ...
        var siblings = SiblingsBlock(comp);
    ...
    /// <summary>
    /// Кто на этой станции свой: остальные роботы того же ИИ, поимённо.
    ///
    /// <para>
    /// Правка по итогам раунда 305 (01.09.2026). Корпусов стало семь, и они начали расстреливать
    /// друг друга: Штык всадил в Клина и Шипа семнадцать зарядов встроенного лазера за полминуты.
    /// Причина не в законах и не в личности, а в том, что робот НЕ ЗНАЛ, кто свой. В наблюдениях
    /// другой корпус приходит строкой вида «crew-4 Клин (Si-1630) | Alive» — то есть ровно так же,
    /// как человек, и до этого блока в промпте не было ничего, что отличало бы одно от другого.
    /// Пока боевой корпус был один, вопрос не возникал.
    /// </para>
    /// <para>
    /// Имена берутся из ЖИВЫХ сессий, а не из прототипа, и это принципиально: их выдаёт аллокатор
    /// по номеру тела, список в прототипе может смениться, а промпт обязан совпадать с тем, что
    /// робот увидит в эфире и в look, посимвольно. По той же причине печатается имя сущности со
    /// всей припиской «(Si-1630)» — сравнивать он будет именно с ней.
    /// </para>
    /// <para>
    /// Блок печатается всегда, даже когда сосед один или их нет вовсе: правило «не бей своих»
    /// должно стоять в замороженном префиксе, а не появляться в нём на середине смены — иначе
    /// первый же новый корпус обнулит кэш всем остальным.
    /// </para>
    /// </summary>
    private string SiblingsBlock(AiBorgComponent comp)
    {
        // Обход ВСЕХ тел с компонентом, а не только заклеймленных, и имена из пула, а не с
        // сущностей. Причина в порядке раунда: правило спавнит семь корпусов, автозахват идёт по
        // одному, и промпт первого собирается тогда, когда остальные ещё безымянны. По живым
        // сессиям первый робот получил бы «кроме тебя никого нет» — то есть ровно ту слепоту,
        // которую блок и лечит. Пул имён известен заранее и от порядка не зависит.
        //
        // Лишнее имя в списке безвредно (запрет бить того, кого нет, ничего не стоит), а
        // недостающее стоит расстрелянного корпуса — поэтому список намеренно широкий.
